            ...
            >
              ...
              {(() => {
                const referencedVideos = (entry.videoIds ?? [])
                  ...
                  .filter(
                    (item): item is { videoId: string; video: VideoCardData } =>
                      Boolean(item),
                  );
                if (!referencedVideos.length) return null;
                return (
                  <SummaryVideoCardList>
                    {referencedVideos.map(({ videoId, video }, videoIndex) => {
                      const meta = resolveVideoMetaInfo(video);
                      return (
                        <SummaryVideoCard
                          key={safeKey(videoId, videoIndex)}
                          href={
                            buildDetailHref?.(video.id, video.href) ||
                            video.href ||
                            `/detail/${video.id}`
                          }
                          onClick={onVideoNavigate}
                        >
                          <VideoSourceContainer>
                            <VideoThumbnailWrapper>
                              <VideoThumbnailImage
...
type SlotDataCache = Record<string, SlotPackage["tabs"] | undefined>;

const makeSlotCacheKey = (sectionId: string, slotId: string) =>
  `${sectionId}::${slotId}`;

const buildInitialSlotCache = (sections: RecapSection[]): SlotDataCache => {
  const cache: SlotDataCache = {};
  sections.filter(isMoneySection).forEach((section) => {
    section.slotPackages.forEach((slot) => {
      if (slot.tabs) {
        cache[makeSlotCacheKey(section.id, slot.id)] = slot.tabs;
      }
    });
  });
  return cache;
...
const resolveCachedSlotTabs = (
  cache: SlotDataCache,
  sectionId: string,
  slotId?: string | null,
) => {
  if (!slotId) return undefined;
  return cache[makeSlotCacheKey(sectionId, slotId)];
};

const normalizeDateForApiParam = (value?: string | null) => {
...
const resolveSectionKey = (section: RecapSection) => {
  if (section.id?.startsWith("section-")) {
    return section.id.replace("section-", "");
  }
  if (section.anchor) return section.anchor;
...
interface StandardBriefingLandingPageClientProps extends Omit<
...
const StandardBriefingLandingPageClient = ({
  ...
  const logVideoInteraction = useCallback(
    (
      ...
      if (slotId) context.slot_key = slotId;
      const userEmailFallback = (() => {
        if (!normalizedPhone) return undefined;
        if (normalizedDateForLogging && action.startsWith("video_card_view")) {
          return `${normalizedDateForLogging}-${normalizedPhone}`;
        }
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (!videoId) return;
          const sectionKeyParam = target.dataset.sectionKey ?? "";
          const slotId = target.dataset.slotId ?? undefined;
          const ratio = entry.intersectionRatio;
          if (ratio >= 0.95 && !loggedVideoViewsRef.current.has(videoId)) {
            loggedVideoViewsRef.current.add(videoId);
            logVideoInteraction(
              "video_card_view_100",
              videoId,
              sectionKeyParam,
  ...
  const buildBriefingDetailHref = useCallback(
    (videoId?: string | null, customHref?: string | null) => {
      if (customHref) {
        if (customHref.startsWith("/detail/")) {
          const detailUrl = new URL(customHref, "http://placeholder.local");
          detailUrl.searchParams.set("from", "briefing");
          if (data.briefingId) {
            detailUrl.searchParams.set("briefingId", data.briefingId);
          }
          if (landingPath) {
            detailUrl.searchParams.set("fromPath", landingPath);
          }
          const query = detailUrl.searchParams.toString();
          return `${detailUrl.pathname}${query ? `?${query}` : ""}`;
        }
        return customHref;
      }
      ...
      if (data.briefingId) {
        params.set("briefingId", data.briefingId);
      }
      if (landingPath) {
        params.set("fromPath", landingPath);
      }
      return `/detail/${videoId}?${params.toString()}`;
    },
    [data.briefingId, landingPath],
  );
  ...
  const diffBadgeByMoneySectionSlot = useMemo(() => {
    ...
    data.sections.filter(isMoneySection).forEach((section) => {
      ...
      filterOutRankingSlots(section.slotPackages).forEach((slot) => {
        ...
        const diff = tabs ? diffCountFromBase(baseIds, tabs.videos) : 0;
        map[section.id][slot.id] = { diff, badge: badgeForDiff(diff) };
      });
    });
    return map;
  ...
  const ensureSlotData = useCallback(
    async (section: MoneyRecapSection, slotId: string) => {
      ...
      const cacheKey = makeSlotCacheKey(section.id, slotId);
      if (slotDataCache[cacheKey]) return;
      if (!section.sourceKey) return;
      setSlotLoadingMap((prev) => ({ ...prev, [cacheKey]: true }));
      try {
        const params = new URLSearchParams({
        ...
        if (payload?.tabs) {
          setSlotDataCache((prev) => ({
            ...prev,
            [cacheKey]: payload.tabs,
          }));
        }
      } catch (error) {
        console.warn("Failed to fetch slot data", error);
        showToast("슬롯 데이터를 불러오지 못했어요");
      } finally {
        setSlotLoadingMap((prev) => {
          const next = { ...prev };
  ...
  useEffect(() => {
    data.sections.filter(isMoneySection).forEach((section) => {
      const defaultSlotId = getDefaultSlotId(section);
      if (
        defaultSlotId &&
        !resolveCachedSlotTabs(slotDataCache, section.id, defaultSlotId)
      ) {
        ensureSlotData(section, defaultSlotId);
      }
    });
  ...
  const adaptSlotVideosToSources = (
    ...
    return videos.map((video) => {
      ...
      return {
        ...
        summary: summaryText,
        href: buildBriefingDetailHref(video.id, video.href),
      } as InsightSource;
    });
  };
  ...
  const buildVideoLookup = (
    sectionId: string,
    packages: SlotPackage[],
    cache: SlotDataCache,
  ) => {
    const map = new Map<string, VideoCardData>();
    packages.forEach((pkg) => {
      const tabs = resolveCachedSlotTabs(cache, sectionId, pkg.id);
      tabs?.videos.forEach((video) => {
        if (video?.id) {
  ...
  const renderMoneySection = (
    ...
    const activeSlotCacheKey = activeSlot
      ...
      : null;
    const isSlotLoading = activeSlotCacheKey
      ? Boolean(slotLoadingMap[activeSlotCacheKey])
      : false;
    const slotLabel: SlotLabel | null = activeSlot
      ? buildSectionSlotLabel(section.title, activeSlot)
      : null;
    const insightSection = activeSlotTabs?.insightSection;
      ...
      if (section.type === "crypto") {
        return (
          <LandingCryptoInsightSection
            ...
            onVideoNavigate={triggerNavigationLoading}
          />
        );
      }
    ...
    const summaryLines = activeSlotTabs?.market.commentary?.length
      ? activeSlotTabs.market.commentary
      : section.summaryBullets;
    const summaryVideoLookup = buildVideoLookup(
      section.id,
      slotPool,
      slotDataCache,
    );
    const sectionKey = resolveSectionKey(section);
    const handleFeedback = async (rating: "good" | "meh" | "bad") => {
      setSlotFeedbackBySection((prev) => ({ ...prev, [sectionKey]: rating }));
      const primaryVideoId = activeSlotTabs?.videos?.[0]?.id;
      const videoIdentifier = primaryVideoId ?? `slot:${sectionKey}`;
      const videoIdWithPhone = normalizedPhone
        ? `${videoIdentifier}__phone:${normalizedPhone}`
        : videoIdentifier;
      try {
        await fetch("https://youticle.shop/editor/first-impressions", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            video_id: videoIdWithPhone,
            reaction: rating,
          }),
        });
      } catch (error) {
        console.warn("Failed to post slot feedback", error);
      }
      const message =
        rating === "good"
          ? "좋아요! 이런 구성을 더 추천드릴게요."
          : rating === "meh"
            ? "의견 고맙습니다. 더 나은 편성을 고민해볼게요."
            : "피드백 감사합니다. 개선에 참고할게요.";
      showToast(message);
    };
    const renderVideoCard = (video: VideoCardData) => {
      ...
      const detailHref =
        buildBriefingDetailHref(video.id, video.href) ?? `/detail/${video.id}`;
      ...
      return (
        <VideoCardObserver
          key={video.id}
          ref={attachVideoObserver(
            video.id,
            sectionKey,
            activeSlot?.id ?? null,
          )}
        >
          <VideoSourceCard href={detailHref} onClick={handleCardClick}>
            <VideoSourceContainer>
              <VideoThumbnailWrapper>
    ...
    const slotVideos = activeSlotTabs?.videos ?? [];
    ...
    let regularVideos = highlightGroups.length
      ? slotVideos.filter((video) => !highlightedVideoIds.has(video.id))
      : slotVideos;
    ...
    const renderVideoEmptyNotice = (variant: "future" | "empty") => {
      if (!activeSlot) return null;
      const slotTitleText =
        slotLabel?.title?.trim() ||
        activeSlot.label?.trim() ||
        section.title.trim();
      const slotDescriptionText =
        slotLabel?.description?.trim() || activeSlot.description?.trim() || "";
      const message =
        variant === "future"
          ? "해당 슬롯에서 선정된 TOP5 영상은 아직 준비 중이에요."
          : "해당 슬롯에서 선정된 TOP5 영상은 오늘은 없습니다.";
      return (
        <VideoEmptyNotice>
          <VideoEmptyNoticeTitle>{slotTitleText}</VideoEmptyNoticeTitle>
          {slotDescriptionText ? (
            <VideoEmptyNoticeDescription>
              {slotDescriptionText}
            </VideoEmptyNoticeDescription>
          ) : null}
          <VideoEmptyNoticeMessage>{message}</VideoEmptyNoticeMessage>
        </VideoEmptyNotice>
      );
    };
    const renderVideoLoadingNotice = () => {
      const slotTitleText =
        slotLabel?.title?.trim() ||
        activeSlot?.label?.trim() ||
        section.title.trim();
    ...
    if (isFutureSlot || isMissingSlot) {
      return {
        ...
        node: isMissingSlot
          ? renderVideoEmptyNotice("empty")
          : renderEmptySlotNotice("pending"),
      };
    }
                ...
                <FeedbackActions>
                  {[
                    ...
                    <FeedbackButton
                      ...
                      $active={slotFeedbackBySection[sectionKey] === option.key}
...
const BriefingLandingPageClient = (props: BriefingLandingPageClientProps) => {
  ...
  return <StandardBriefingLandingPageClient {...rest} data={data} />;
};
...
const VideoEmptyNotice = styled(InsightEmptyState)`
  margin-top: 16px;
`;

const VideoEmptyNoticeTitle = styled.strong`
  display: block;
  font-size: 14px;
  color: #1f2937;
`;
...
const VideoEmptyNoticeMessage = styled.p`
  margin-top: 10px;
  font-size: 13px;
  color: #475569;
`;
...
const FeedbackButton = styled.button<{ $active: boolean }>`
  ...
  border: 1px solid
    ${({ $active }) =>
      $active ? "rgba(59,130,246,0.6)" : "rgba(15,23,42,0.12)"};
  background: ${({ $active }) =>
    $active ? "rgba(59,130,246,0.08)" : "rgba(255,255,255,0.9)"};
  font-weight: 700;
  font-size: 14px;
  color: #1f2a4a;
  cursor: pointer;

  ...
  return (
    <Page>
      ...
      <Hero ref={heroRef}>
        <HeroInner>
          <HeroTitle>
            이제 유튜브 검색·시청 한 시간 대신, <br />
            5분이면 충분합니다. <br /> <br />
            <HeroTitleSub as="span">
              내 유튜브 키워드 TOP5 영상 브리핑으로 <br /> 핵심만 매일 챙기세요.
            </HeroTitleSub>
          </HeroTitle>
          {/* <HeroDesc>
            국내·해외 수백 개 영상 중 노이즈를 걷어내고, <br />
            데이터로 선별한 <b>TOP5만 요약</b>해 드려요. <br />
      ...
      {/* 프리뷰 섹션 */}
      <Section>
        <SectionTitle>📨 오늘 이렇게 이메일 도착해요</SectionTitle>
        ...
        <HeroVideoWrapper ref={previewRef}>
          ...
          >
            <source src="/videos/output_4mb.mp4" type="video/mp4" />
      ...
      {/* 선별 과정 3단계 (설명 업데이트) */}
      <Section>
        <SectionTitle>어떻게 TOP5 영상을 선별하나요?</SectionTitle>
        <Sub>잡음을 덜고, 쓸모만 남기는 3단계</Sub>

        <Steps>
          <StepCard>
            <StepBadge>01</StepBadge>
            <StepTitle>후보 수집 · 중복·낚시 제거</StepTitle>
            <StepDesc>
              국내·해외 주요 채널에서 자동 수집 후 제목 낚시와 중복 콘텐츠를
              제거합니다.
            </StepDesc>
          </StepCard>
          <StepCard>
            <StepBadge>02</StepBadge>
            <StepTitle>시청자 반응·최신도 스코어링</StepTitle>
            <StepDesc>
              조회·좋아요·댓글 등 반응과 업로드 최신도를 반영한 점수로 순위를
              매깁니다.
            </StepDesc>
          </StepCard>
          <StepCard>
            <StepBadge>03</StepBadge>
            <StepTitle>TOP5 확정 · 3줄 요약</StepTitle>
            <StepDesc>
              상위 5개만 선정해 영상당 핵심 3줄과 출처 링크를 제공합니다.
            </StepDesc>
          </StepCard>
        </Steps>
      </Section>
      ...
      {/* 신뢰/안심 요소 (레이아웃 확장) */}
      {/* 푸터형 신뢰 + 전환 훅 (이메일 썸네일 포함) */}
      <FooterSection as="footer">
        <FooterInner>
          <FooterLeft>
            <FooterTitle>
              유튜브 TOP5 최신 영상, 각 3줄 요약으로 매일 아침 도착합니다.
              <br />
              <span>검색 피로 없이 핵심만 빠르게 확인하세요.</span>
            </FooterTitle>
            <FooterRight>
              <EmailPreviewCard>
                <EmailThumb
                  src="/images/이메일브리핑썸네일.png"
                  alt="이메일 브리핑 썸네일 예시"
                  onError={(e) => {
                    (e.target as HTMLImageElement).src =
                      "/images/email-briefing-thumb.png";
                  }}
                />
              </EmailPreviewCard>
            </FooterRight>
            <FooterTrustRow>
              <TrustItem>⚡ TOP5 최신 영상 × 각 3줄 요약</TrustItem>
              <TrustItem>🕖 매일 7:30–9:00 자동 도착</TrustItem>
              <TrustItem>🔒 스팸·광고 없음 · 1클릭 해지</TrustItem>
            </FooterTrustRow>
          </FooterLeft>
        </FooterInner>
      </FooterSection>
      {/* Sticky CTA */}
      <StickyBar data-visible={showSticky} aria-hidden={!showSticky}>
        <StickyBtn onClick={openPicker}>
          키워드 고르고 바로 브리핑 받기
        </StickyBtn>
      </StickyBar>
      ...
      {pickerOpen && (
        <Modal onClick={() => setPickerOpen(false)} aria-modal>
          <ModalBox onClick={(e) => e.stopPropagation()}>
            <CloseBtn aria-label="닫기" onClick={() => setPickerOpen(false)}>
              ×
            </CloseBtn>
...
const ModalHeader = styled.div`
  padding: 14px 14px 10px; /* ⬅︎ 좌우 패딩 추가 */
...
const FooterTitle = styled.h3`
  ...
  /* font-size: 20px; */
`;
const FooterTrustRow = styled.div`
  display: grid;
  grid-template-columns: 1fr;
  gap: 10px;
  margin: 40px 0 16px;
